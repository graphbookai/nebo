...
// Sticky ruler band: tick labels + a playhead marker, sharing the canvas X
// transform so labels line up with the datapoint columns below.
export function TimelineRuler({ ticks, axis, isStep, minTime, height, playheadPct }: {
  ticks: number[]
  axis: AxisTransform
  isStep: boolean
  minTime: number
...
// blank spacers so the rows stay aligned 1:1 with the tree column), tick
// guides, and the playhead. SVG layers are pointer-transparent so the parent
// handles scrub/pan/zoom.
export function TimelineRows({ rows, rowHeight, isStep, axis, ticks, playheadPct }: {
  rows: FlatRow[]
  rowHeight: number
  isStep: boolean
  axis: AxisTransform
